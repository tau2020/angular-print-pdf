				...
				link : function(scope , element , attrs ) {
					...
					function convert(theElement , id) {
				        ...
				        html2canvas(quotes, {
				            ...
				            var pdf = new jsPDF('p', 'px', [841.89, 1190.55]);
				            for (var i = 0; i <= quotes.clientHeight/1190.55 ; i++) {
				                var srcImg  = canvas;
				                var sX      = 0;
				                var sY      =1750 *i; // start1190.55  pixels down for every new page
				                var sWidth  = 1100  ;
				                var sHeight =1750 ;
				                var dX      = 0;
				                var dY      = 0;
				                ...
				                onePageCanvas.setAttribute('height',1750 );
				                ...
				                // add another page
				                if (i > 0) {
				                    pdf.addPage(841.89, 1190.55); //8.5" x 11" in pts (in*72)
				                }
					...
					/*	var element = $('[pdf-save-content='+id+']') ,
						cache_width = element.width(),
						 a2  =[ 841.89, 841.89];  // for a2 size paper width and height
